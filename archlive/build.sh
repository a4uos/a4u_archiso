...
iso_name=a4u
iso_label="A4U_$(date +%Y%m)"
iso_publisher="A4u Linux <https://ordanax.github.io>"
iso_application="A4U Linux Live/USB"
...
make_packages() {
    $MKARCH_ISO ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.x86_64)" install
}
...
# Build ISO
make_iso() {
    $MKARCH_ISO ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -P "${iso_publisher}" -A "${iso_application}" -o "${out_dir}" iso "${iso_name}-${iso_version}-x86_64.iso"
}
